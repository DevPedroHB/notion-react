...
export function Editor() {
  ...
  return (
    <>
      <EditorContent
        className="max-w-[43.75rem] mx-auto pt-16 prose prose-violet"
        editor={editor}
      />
      {editor && (
        <FloatingMenu
          editor={editor}
          className="bg-zinc-100 py-2 px-1 shadow-xl border gap-1 border-zinc-200 shadow-black/20 rounded-lg overflow-hidden flex flex-col"
          shouldShow={({ state }) => {
      ...
      {editor && (
        <BubbleMenu
          className="bg-zinc-100 shadow-xl border border-zinc-200 shadow-black/20 rounded-lg overflow-hidden flex divide-x divide-zinc-300"
          editor={editor}
        >
          <BubbleButton>
            Text
